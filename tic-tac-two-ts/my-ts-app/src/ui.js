...
function dispatchActionChangeEvent(actionType) {
    const customEvent = new CustomEvent("actionChange", {
        detail: { actionType }
    });
...
export function switchToGameView() {
    var _a, _b;
    (_a = document.getElementById("mode-selection")) === null || _a === void 0 ? void 0 : _a.classList.add("hidden");
    (_b = document.getElementById("game-container")) === null || _b === void 0 ? void 0 : _b.classList.remove("hidden");
}
export function switchToMenuView() {
    var _a, _b;
    (_a = document.getElementById("game-container")) === null || _a === void 0 ? void 0 : _a.classList.add("hidden");
    (_b = document.getElementById("mode-selection")) === null || _b === void 0 ? void 0 : _b.classList.remove("hidden");
}
export function createBoard(game, onCellClick) {
    if (!boardElement)
        return;
...
export function updateGameInfo(game) {
    ...
    const currentActionType = game.actionType;
    if (currentActionType === ActionType.PLACE) {
        (_a = document.getElementById("place-button")) === null || _a === void 0 ? void 0 : _a.classList.add("active");
    }
    else if (currentActionType === ActionType.MOVE_PIECE) {
        (_b = document.getElementById("move-button")) === null || _b === void 0 ? void 0 : _b.classList.add("active");
    }
    else if (currentActionType === ActionType.MOVE_GRID) {
        (_c = document.getElementById("grid-button")) === null || _c === void 0 ? void 0 : _c.classList.add("active");
    }
    ...
    const gridMoveButton = document.getElementById("grid-button");
    if (game.playerPieces[game.currentPlayer] >= 3 && game.playerPieces[game.currentPlayer === "X" ? "O" : "X"] >= 3) {
        ...
        if (gridMoveButton)
            gridMoveButton.disabled = false;
    }
    else {
        if (placeButton)
            placeButton.disabled = game.playerPieces[game.currentPlayer] >= 3;
        if (moveButton)
            moveButton.disabled = true;
        if (gridMoveButton)
            gridMoveButton.disabled = true;
    }
...
export function setupGridControls(onGridMove) {
    if (!gridControlsElement)
        return;
    const buttons = gridControlsElement.querySelectorAll(".direction-button");
    buttons.forEach(button => {
        const htmlButton = button;
        const direction = htmlButton.dataset.direction;
        if (direction) {
...
export function addButtonEventListener(id, eventType, listener) {
